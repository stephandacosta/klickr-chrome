...
Player.prototype.processData = function(arr, index, xScale, yScale){
  ...
  if ( index === arr.length ) {
    ...
    Player.move(arr[index].clientX, arr[index].clientY ,arr[index].t);
    setTimeout(function(){
      Player.processData(arr, index+1);
    }, arr[index].t );
  }
};
...
Player.prototype.playRecording = function(){
  ...
  Player.processData(movement, 1, xScale, yScale);
};

Player.prototype.playKlick = function(url, clickId){
  url = url || '';
  clickId = clickId || '';
